...
// Extend Express Request interface to include user and cookies
declare global {
    namespace Express {
        interface Request {
            user?: {
 ...
 */
function extractTokenCookie(cookies: any): string | null {
 ...
 * @returns Express middleware function
 */
export function requireTopicAccess(topicVisibility: string, topicUserId: string) {
    return (req: Request, res: Response, next: NextFunction): void => {
        try {
