...
function getErrorStatusCode(error: Error): number {
    ...
    if (error instanceof jwt.JsonWebTokenError) return 401;
    if (error instanceof jwt.TokenExpiredError) return 401;
...
function getErrorCode(error: Error): string {
    ...
    if (error instanceof BadRequestError) return 'BAD_REQUEST_ERROR';
 ...
 */
export function validationErrorHandler(error: Error, req: Request, res: Response, next: NextFunction): void {
    if (error instanceof ZodError) {
 ...
 */
export function jwtErrorHandler(error: Error, req: Request, res: Response, next: NextFunction): void {
    if (error instanceof jwt.JsonWebTokenError) {
