    ...
    return (req, res, next) => {
        try {
            ...
            if (error instanceof ZodError) {
                const validationError = new ValidationError('Validation failed', { errors: error.errors });
...
export function validateQuery(schema) {
    return (req, res, next) => {
        try {
            ...
            if (error instanceof ZodError) {
                const validationError = new ValidationError('Query validation failed', { errors: error.errors });
                return res.status(400).json(validationError.serialize());
            }
            next(error);
        }
...
export function validateParams(schema) {
    return (req, res, next) => {
        try {
            ...
            if (error instanceof ZodError) {
                const validationError = new ValidationError('Parameter validation failed', { errors: error.errors });
                return res.status(400).json(validationError.serialize());
            }
            next(error);
        }
