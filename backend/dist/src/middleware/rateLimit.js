 ...
 */
async function getRateLimitInfo(key, limit, windowMs) {
    const now = Date.now();
...
export function rateLimit(config) {
    const { windowMs = env.RATE_LIMIT_WINDOW_MS, maxRequests = env.RATE_LIMIT_MAX_REQUESTS, keyGenerator = defaultKeyGenerator, skipSuccessfulRequests = false, skipFailedRequests = false, message = 'Rate limit exceeded' } = config;
            ...
            if (skipSuccessfulRequests || skipFailedRequests) {
                ...
                res.send = function (body) {
                    ...
                    if ((isSuccess && skipSuccessfulRequests) || (!isSuccess && skipFailedRequests)) {
                        ...
                        redisClient.zRem(key, Date.now().toString()).catch((error) => {
 ...
 */
export async function getAllRateLimitStatus(req) {
    const status = {};
    try {
